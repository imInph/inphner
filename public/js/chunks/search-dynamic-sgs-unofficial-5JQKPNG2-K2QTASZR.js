import{a as T}from"./chunk-2F3GERFI.js";import{m as x}from"./chunk-TVSZI2UX.js";import"./chunk-PGV3QAIX.js";import{a as m}from"./chunk-2UI6XZXJ.js";import{s as b}from"./chunk-MXUW4ABC.js";import"./chunk-FY3BTKY4.js";function p(R){if(R<2)return R;let B=1;for(let L=2;L<=R;L++)B*=L;return B}function v(R,B){if(B<0||B>R)return 0;if(B===0||B===R)return 1;let L=1;for(let F=0;F<B;F++)L=L*(R-F)/(F+1)|0;return L}function J(R){R=R.slice();let B=R.length,L=p(B-1),F=0;for(;B>1;){B--;let l=R[0];F+=l*L;for(let U=0;U<B;U++){let g=R[U+1];R[U]=g-(g>l)}L/=B}return F}function K(R,B){let L=[],F=p(B-1);for(let l=0;l<B;l++)L[l]=R/F|0,R%=F,F/=B-1-l;for(let l=B-2;l>=0;l--)for(let U=l+1;U<B;U++)L[U]+=+(L[U]>=L[l]);return L}function M(R){let B=R.length,L=0;for(let F=0;F<B-1;F++)for(let l=F;l<B;l++)R[F]>R[l]&&(L^=1);return L}function P(R,B){let L=[],F=p(B-1)/2,l=0;for(let U=0;U<B-1;U++)L[U]=R/F|0,R%=F,F/=B-1-U;L[B-1]=0;for(let U=B-2;U>=0;U--)for(let g=U+1;g<B;g++)L[g]>=L[U]?L[g]++:l^=1;return l===1&&([L[B-2],L[B-1]]=[L[B-1],L[B-2]]),L}function S(R){return J(R)>>1}function j(R){let B=R.length,L=0;for(let g=0;g<B;g++)L+=+(R[g]===1);let F=B-L;if(F===0||L===0||B===1)return 0;let l=v(B-1,L),U=0;for(let g=0;F>0&&L>0&&B>1;g++)B--,R[g]===0?l=l*--F/B:(U+=l,l=l*L--/B);return U}function Q(R,B,L){let F=L-B,l=v(L-1,B),U=[],g=L-1;for(let A=0;A<g;A++)L--,R<l?(U.push(0),l=l*--F/L):(U.push(1),R-=l,l=l*B--/L);return U.push(B),U}function s(R,B){let L=[];for(let F=0;F<B.length;F++)L[F]=R[B[F]];return L}function i(R,B){let L=[];for(let F=0;F<B;F++)L[F]=F;for(let F=0;F<R.length;F++)L[R[F]]=R[(F+1)%R.length];return L}function f(R,B){let L=Array(B).fill(0);for(let F in R)L[F]=R[F];return L}function V(R,B){let L=Array(8).fill(0);for(let F=0;F<8;F++)L[F]=(R[1][F]+B[1][F])%3;return[s(R[0],B[0]),L]}var W=[i([0,1,4],12),f({0:2},8)],X=[i([1,2,5],12),f({1:2},8)],Y=[i([2,3,6],12),f({2:2},8)],Z=[i([3,0,7],12),f({3:2},8)],$=[i([9,8,4],12),f({4:2},8)],BB=[i([10,9,5],12),f({5:2},8)],LB=[i([11,10,6],12),f({6:2},8)],RB=[i([8,11,7],12),f({7:2},8)],vB=[K(0,12),Array(8).fill(0)],h=[W,X,Y,Z,$,BB,LB,RB],FB=["UL","U","UR","F","L","B","R","D"],_=[0,1,0,1,1,0,1,0];function UB(R){let B=["0","","'"];return R.map(([F,l])=>FB[F]+B[l]).join(" ")}function lB(){let R=P(Math.floor(m(p(12))/2),12),B=Array(8);for(let L=0;L<8;L++)B[L]=m(3);return[R,B]}function DB(){return gB(lB())}function gB(R){let B=AB(R),L=[C(),k()],F=[tB(),rB()],l=[O(),nB()],U=[oB(),eB()],g=iB(B,L,F),A,D=new Set,r=+new Date;for(let o=0;o<25;o++){let{value:t,done:n}=g.next(),a=R;for(let[u,E]of t)for(let y=0;y<E;y++)a=V(a,h[u]);if(D.has(a.toString()))continue;D.add(a.toString());let I=S(a[0].slice(0,8)),w=0;for(let u=0;u<4;u++)w+=a[1][u]*3**u;let H=[I,w],N=A?A.length-t.length-1:999999,c=aB(H,l,U,N);if(c!==void 0&&((A===void 0||A.length>t.length+c.length)&&(A=t.concat(c)),new Date-r>300))break}return A}function AB(R){let B=0;for(let U=0;U<4;U++)B+=R[0].indexOf(U+8)*12**U;let L=0;for(let U=0;U<4;U++)L+=R[1][U+4]*3**U;let F=R[0].map(U=>Math.max(-1,U-8)),l=j(F.map(U=>+(U>=0)))*2+M(F.filter(U=>U>=0));return[B,L+81*l]}var e={};function z(){if(e.phase1cm)return e.phase1cm;let R=[];for(let B=0;B<81;B++){R[B]=Array(8);let L=[B%3,Math.floor(B/3)%3,Math.floor(B/9)%3,Math.floor(B/27)];R[B][0]=R[B][1]=R[B][2]=R[B][3]=B;for(let F=0;F<4;F++)L[F]=(L[F]+2)%3,R[B][4+F]=L[0]+L[1]*3+L[2]*9+L[3]*27,L[F]=(L[F]+1)%3}return e.phase1cm=R}function eB(){if(e.phase1cp)return e.phase1cp;let R=Array(81);for(let B=0;B<81;B++){let L=[B%3,Math.floor(B/3)%3,Math.floor(B/9)%3,Math.floor(B/27)];R[B]=(L[0]!==0)+(L[1]!==0)+(L[2]!==0)+(L[3]!==0)}return e.phase1cp=R}function C(){if(e.phase1em)return e.phase1em;let R=[];for(let L=0;L<12;L++){R[L]=[];for(let F=0;F<8;F++)R[L][F]=h[F][0].indexOf(L)}let B=Array(12**4);for(let L=0;L<12**4;L++){B[L]=Array(8);for(let F=0;F<8;F++){let l=0;for(let U=0;U<4;U++)l+=R[Math.floor(L/12**U)%12][F]*12**U;B[L][F]=l}}return e.phase1em=B}function tB(){return e.phase1ep?e.phase1ep:e.phase1ep=d(C(),[8+12*1713])}function k(){if(e.phase1sm)return e.phase1sm;let R=v(12,4),B=[];for(let l=0;l<R;l++){B[l]=[];let U=Q(l,4,12),g=[];for(let A=0,D=0;A<12;A++)U[A]===0?g[A]=-1:g[A]=D++;for(let A=0;A<8;A++){let D=s(g,h[A][0]),r=s(U,h[A][0]),o=M(D.filter(t=>t>=0));B[l][A]=j(r)*2+o}}let L=z(),F=[];for(let l=0;l<R;l++)for(let U=0;U<81;U++){let g=F[U+81*(2*l)]=[],A=F[U+81*(2*l+1)]=[];for(let D=0;D<8;D++)g[D]=L[U][D]+81*B[l][D],A[D]=L[U][D]+81*(B[l][D]^1)}return e.phase1sm=F}function rB(){return e.phase1sp?e.phase1sp:e.phase1sp=d(k(),[0])}function nB(){if(e.phase2cm)return e.phase2cm;let R=z(),B=Array(81);for(let L=0;L<81;L++)B[L]=R[L].slice(4,8);return e.phase2cm=B}function O(){if(e.phase2em)return e.phase2em;let R=8,B=p(R)/2,L=Array(B),F=[0,1,2,3,4,5,6,7];for(let l=0;l<B;l++){L[l]=Array(4);for(let g=0;g<4;g++){let A=s(F,h[g][0].slice(0,8));L[l][g]=S(A)}if(l===B-1)break;let U=0;do for(let g=R-2;g>=0;g--){if(F[g]>F[g+1])continue;let A=g+1;for(let D=A;D<R;D++)F[D]>F[g]&&(A=D);[F[g],F[A]]=[F[A],F[g]],U^=1;for(let D=0;g+1+D<R-1-D;D++,U^=1)[F[g+1+D],F[R-1-D]]=[F[R-1-D],F[g+1+D]];break}while(U!==0)}return e.phase2em=L}function oB(){return e.phase2ep?e.phase2ep:e.phase2ep=d(O(),[0])}function d(R,B){let L=R.length,F=R[0].length,l=Array(L).fill(-1),U=B.slice(),g=[],A=0;for(;U.length>0;){g.length=0;for(let D of U)if(l[D]===-1){l[D]=A;for(let r=0;r<F;r++){let o=R[D][r];for(;o!==D;)g.push(o),o=R[o][r]}}[U,g]=[g,U],A+=1}return l}function aB(R,B,L,F){F=F||999999;let l=R.length,U=0;for(let g=0;g<l;g++)U=Math.max(U,L[g][R[g]]);for(;U<=F;){let g=q(R,B,L,U,-1);if(g!==void 0)return g;U++}}function q(R,B,L,F,l){let U=R.length,g=B[0][0].length,A=0;for(let D=0;D<U;D++)A=Math.max(A,L[D][R[D]]);if(!(A>F)){if(F===0)return[];if(!(A===0&&F===1))for(let D=0;D<g;D++){if(D===l||D<l&&_[D]===_[l])continue;let r=R.slice();for(let t=0;t<U;t++)r[t]=B[t][R[t]][D];let o=1;for(;R.some((t,n)=>R[n]!==r[n]);){let t=q(r,B,L,F-1,D);if(t!==void 0)return[[D,o]].concat(t);for(let n=0;n<U;n++)r[n]=B[n][r[n]][D];o++}}}}function*iB(R,B,L){let F=R.length,l=0;for(let U=0;U<F;U++)l=Math.max(l,L[U][R[U]]);for(;;)yield*G(R,B,L,l,-1),l++}function*G(R,B,L,F,l){let U=R.length,g=B[0][0].length,A=0;for(let D=0;D<U;D++)A=Math.max(A,L[D][R[D]]);if(!(A>F)){if(F===0){yield[];return}if(!(A===0&&F===1))for(let D=0;D<g;D++){if(D===l||D<l&&_[D]===_[l])continue;let r=R.slice();for(let t=0;t<U;t++)r[t]=B[t][R[t]][D];let o=1;for(;R.some((t,n)=>R[n]!==r[n]);){let t=G(r,B,L,F-1,D);for(;;){let{value:n,done:a}=t.next();if(a)break;yield[[D,o]].concat(n)}for(let n=0;n<U;n++)r[n]=B[n][r[n]][D];o++}}}}async function dB(){return new b(UB(DB()))}var fB=null;async function wB(){return fB??=uB()}async function uB(){return T(await x.fto.kpuzzle(),`SubgroupSizes 24 12 11 12 12 11 10 9 11 8 10 9 7 8 10 10 6 9 8 5 7 6 5 4 4 8 7 3 6 6 360
...
`)}export{dB as getRandomRediCubeScramble,wB as sgsDataFTO};
//# sourceMappingURL=search-dynamic-sgs-unofficial-5JQKPNG2-K2QTASZR.js.map
